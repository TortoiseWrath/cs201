...
static int rcompareREAL(REAL *a, REAL *b);
static int rcompareSTRING(STRING *a, STRING *b);
static int rcompareINTEGER(INTEGER *a, INTEGER *b);
...
int main(int argc, char **argv) {
	...
	if(sortReals) {
		release = freeREAL;
		display = displayREAL;
		compare = sortDecreasing ? rcompareREAL : compareREAL;
		...
		HEAP *h = newHEAP(display, compare, release);
		while(!feof(fp)) {
			insertHEAP(h, newINTEGER(readInteger(fp)));
		}
	}
...
static void giveExplanation() {
	puts("Written by Sean Gillen <sdgillen@crimson.ua.edu>\n");
